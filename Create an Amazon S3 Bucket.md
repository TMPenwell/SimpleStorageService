...
1\. Amazon S3 (Simple Storage Service) is a scalable, high-speed, web-based cloud storage service. It is designed for online backup and archiving of data and application programs.
...
2\. Go to [aws.amazon.com](http://aws.amazon.com) and sign in. In the search field, type *S3* and select it from the display.
...
3\. In the S3 console, select **Create bucket**
...
4\. Under **General Configuration** you can choose between *General purpose* and *Directory*.
...
5\. There are two bucket types. The General purpose one is used in most cases. Review the information below for more details on the difference between these two types of buckets.
...
6\. Enter a globally unique bucket name.  Bucket names must meet naming requirements and must be unique from any other bucket name in any other AWS account.
...
7\. Select **Object Ownership** between *ACLs disabled* and *ACLs enabled.*
...
8\. Each bucket and object has an ACL attached to it and defines AWS accounts or groups are granted access and what type of access.
...
9\. Adjust settings as needed to **Block Public Access settings for this bucket**.
...
10\. Decide if you want to enable **Bucket Versioning**
...
11\. Benefits of using versioning
...
12\. Remember that billing in S3 is a pay-as-you-go service and charges are based on the objects stored. The rate depends on the size of the object, how long it is stored, and storage class. The rates apply for every version of an object that is stored. ***Each version of an object is the entire object.***
...
13\. **Tags** can be added. A tag is label consisting of a key and (optional) value to store additional data (metadata) about the resource.

Tags can be used to filter and search for resources, monitor cost and usage, and manage the AWS environment.

Tags are not required but is a best practice as as part of building a Well-Architected Framework. Tags can be added when a resource is created or after it is launched.  We will not be adding tags in this demo.
...
14\. Tags can be added to new objects as they are uploaded or can be added to existing objects as needed.
...
15\. Encryption is a best practice and is automatically applied to S3 buckets. You can customize the **Default encryption** to one of three different types.
...
16\. In January 2023, Amazon began encrypting S3 buckets by default. New objects are automatically encrypted by using server-side encryption with Amazon S3 managed keys (SSE-S3) and is at no cost.
...
17\. Objects uploaded before default encryption was enabled  (January 2023) will not be encrypted.


18\. Make a selection on the **Bucket Key** to enable or disable.
...
19\. Click **Create bucket** to create the bucket.
...
20\. If you bucket was created successfully, a green batter will appear at the top of the screen. 
...
21\. With the newly created bucket, we see there are tabs and other options to use this storage service.
...
22\. Objects in a bucket can be thought similarly as files you add to your local file folder or upload in other platforms such as Google Drive. The bucket - object relationships is similar to the folder-file relationship.


23\. To upload files into the bucket, verify you are in the **Objects** tab is selected and click **Upload**
...
24\. Click **Add files** to find the files (objects) to upload into the bucket. You will see a window (File Explorer, Finder, etc.) to select the files to upload.
...
26\. Optionally, you can adjust the *Permissions* and *Properties* for the uploaded objects.
...
27\. Click **Upload** to upload the files into the S3 bucket
...
28\. The green banner at the top indicates that files were successfully uploaded.
...
29\. Click **Close** to exit this screen and return to the bucket page
...
30\. Explore the additional options available for the bucket. Begin with first one - Metadata.
...
32\. The **Properties** tab includes options to information about the bucket. Information found in this tab includes the following
...
33\. You can edit the the **Bucket Versioning** settings by clicking *Edit*
...
34\. You can choose to Enable bucket versioning (recall, it was left disabled when the bucket was created for this demo).
...
35\. If you make changes to versioning, click **Save changes**
...
36\. Continuing with the options under Properties, we see there is a section for **Intelligent-Tiering Archive configuration**. 
...
37\. Intelligent-Tiering Archive configuration is [[a feature that allows you to automatically move objects to more cost-effective archive tiers based on access patterns. It moves objects that have not been access for at least 90 days to archive tiers]]
...
38\. To configure, you begin by creating a name for the configuration. The name should be descriptive of its purpose.
...
39\. Select appropriate **Archive rule actions** to *Archive Access* tier or *Deep Archive Access* tier.
...
42\. Use the breadcrumbs to navigate back to the bucket page.
...
43\. Next, you will go to the **Permissions** tab
...
44\. The Permissions tab in an S3 bucket is used for managing access control and security settings.


45\. Recall that when this bucket was created, the default settings were left on that blocked all public access.
...
46\. You can customize the level of access for the bucket at a granular level based on the same options as provided when you first created the bucket.
...
47\. To edit these bucket settings, click on the **Edit** button
...
48\. This bucket contains images that we want publicly accessible so uncheck the box that says *Block all public access.* This removes a layer of security of who can access the bucket.
...
49\. Unchecking "Block all public access" for an S3 bucket is a significant action that should not be taken lightly. 
...
51\. Follow the instructions to confirm the changes
...
52\. The green banner at top of the screen confirms the changes were made successfully.
...
53\. To allow the appropriate access to this bucket, a **Bucket policy** is required. Select the *Edit* button.
...
54\. A bucket policy is a resource-based policy that you can use to grant access permissions to your Amazon S3 bucket and the objects in it.

Bucket policies are written in JSON and provide fine-grained control over S3 resources.

Bucket polices are used to
...
- state specific conditions as to when to apply the policy (e.g. IP ranges, time of day, etc.)
...
55\. There are a few different ways that you can write the policy.  In this demo, you will click on **Policy generator** which will open a separate tab.
...
56\. For **Select Type of Policy** select *S3 Bucket Policy*
...
57\. The *Principal* is the account, user, role, or service to which the policy applies

An asterisk (\*) can be used as a wildcard which allow anyone to access.
...
58\. The **Actions** are used to identify what actions the principal can do.
...
59\. Search for and select the **GetObject** option.
...
60\. The **GetObject** option is used to grant permission to read (retrieve) objects from Amazon S3 bucket.
...
61\. The **Amazon Resource Name (ARN)** is used as a unique identifier for AWS resources and services. 
...
62\. To find the ARN for this bucket, go back to the S3 console.
...
63\. Find the **Bucket ARN** and copy the arn code
...
64\. Back in the Policy Generator tab, paste the copied ARN into the ARN text box and click **Add Statement**
...
65\. To create the policy, click **Generate Policy**
...
66\. A policy written in JSON is available to be copied.
...
67\. Paste the copied JSON code into the Policy window
...
68\. The **Resource** is not yet complete. You need to specify which object(s) in the bucket are accessible in this policy.  If all objects should be accessible, you can use the wildcard by adding a /\* as shown below.
...
70\. **Structure of a Bucket Policy**
...
71\. A green banner appears confirming changes were made successfully.
...
72\. **Object Ownership** is an Amazon S3 bucket-level setting that you can use to control ownership of objects uploaded to your bucket. 
...
73\. Object Ownership is set to the Bucket owner by default and all ACLs are disabled. When ACLs are disabled, the bucket owner owns all the objects in the bucket and manages access to data exclusively policies.
...
74\. **Access control list (ACL)** are used to manage access to buckets and objects. 
...
75\. The **Grantee** information allows you to see permissions based on *Objects* and *Bucket ACL* layers
...
76\. The Grantee information in Amazon S3 ACLs refers to the entity (user, account, or predefined group) that is granted specific permissions.

When ACLs are disabled in Amazon S3, permissions are still granted through bucket or IAM policies.


77\. The **Cross-origin resource sharing (CORS)** can be used to define a way for client web applications that are loaded in one domain to interact with resources in a different domain.
...
78\. Cross-Origin Resource Sharing (CORS) is an important security feature implemented by web browsers to protect users from potentially malicious cross-origin requests.
...
79\. The Metrics tab provides information on the usage, performance, and health of your S3 buckets. They become available about 15 minutes after configuration.
...
80\. Amazon S3 offers three main types of metrics:
...
Benefits of Bucket Metrics are as follow
...
81\. The **Storage Class Analysis** is used to analyze storage access patterns. This helps you decide when to transition the right data to the right storage class.
...
82\. With **Replication Metrics**, you can monitor the progress of replication. It tracks the following
...
83\. Click on the **Management** tab provides options for configuring various aspects of bucket management and optimization
...
84\. In the Management tab you will be able to configure the following
...
85\. Under Management, the **Lifecycle configuration** is used to create a set of rules that define actions that Amazon S3 applies to a group of objects. 
...
86\. To create a lifecycle rule, click **Create lifecycle rule**
...
87\. Provide a descriptive name for the lifecycle rule.
...
88\. **Choose a rule scope** based on need. Options include
...
90\. **Lifecycle rule actions** are used to set the actions that this rule policy will perform.
...
92\. **Transition current version of objects between storage classes** is used to move the current version of objects.
...
93\. **Transition noncurrent versions of objects between storage classes** is used to move the previous (noncurrent) versions of an object.
...
94\. Set the days as appropriate based on needs. Remember that multiple versions will increase your usage of the storage service and your bill.
...
96\. Green banner confirms the rule was created.
...
98\. There are two types of replication: *live replication* and *on-demand replication*.
...
99\. **Replication rules** are used to enable automatic, asynchronous copying of objects across Amazon S3 buckets.
...
100\. Buckets that are configured for object replication can be owned by the same AWS account or by different accounts.
...
101\. Access Points in Amazon S3 are unique network endpoints that simplify data access for shared datasets in S3 buckets. They provide a flexible way to manage access to your S3 data at scale.


102\. Click the **Access Points** tab
...
103\. This demo is not going to create an access point but you can see the button to begin. Learn more about access points at <https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-access-points.html>
